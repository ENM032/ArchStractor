...
5. [Verifying Your Data](#5-verifying-your-data)
6. [Understanding Backups and Logs](#6-understanding-backups-and-logs)

---

## 1. What is ArcStractor?
ArcStractor is a simple command-line tool that fetches historical South African National Lottery results from the web (specifically PowerBall and PowerBall Xtra archives), validates that the results are correct, and saves them into a text file in a standardized format. 

If the text file already contains data, the tool is smart enough to start exactly where the existing data stops without introducing duplicate draws.
...
### Option A: Standard Run (Original PowerBall)
...
### Option B: Retrieve a Different Game (PowerBall Xtra)
If you want to extract a different game like **PowerBall Xtra**, starting from a specific year range (e.g. 2015 to 2026), and save it to a new file (e.g. `data/dataset_xtra.txt`), run:

```bash
python src/main.py --game powerball-xtra --start-year 2015 --end-year 2026 --output data/dataset_xtra.txt
```
...
- `--output` (or `-o`): The path where the text file will be saved (e.g., `data/my_results.txt`).

---

## 5. Verifying Your Data
...
If you want to verify a custom output file instead (like `data/dataset_xtra.txt`), type:
```bash
python src/verify.py data/dataset_xtra.txt
```
...
## 6. Understanding Backups and Logs
